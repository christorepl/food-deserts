...
export default class RenderSaveResults extends React.Component {
  ...
  render() {
    const statesResults = this.context.currentSaveResults.map((state) => {
      let currentState = this.context.allStates.find(
        (states) => states.id === state[0].id
      );
      return (
        <div
          className="state-container"
          key={`container-${state[0].state_name}-${state[0].fips}`}
        >
          ...
          <h2>
            Total population: {new Intl.NumberFormat().format(state[0].pop)}
          </h2>
          ...
          <li key={`hispanic${state[0].fips}`}>
            Hispanic: {state[0].hispanic} %
          </li>
          <li
            className="ranking-list"
          ...
          <li key={`mixed${state[0].fips}`}>
            Mixed Race: {state[0].mixed_race} %
          </li>
          <li
            className="ranking-list"
          ...
          <li key={`trump${state[0].fips}`}>
            Donald Trump: {state[0].trump} %
          </li>
          <li
            className="ranking-list"
          ...
          <h3> COVID: </h3>
          <li key={`covid_infections${state[0].fips}`}>
            Number of confirmed COVID cases:
            {new Intl.NumberFormat().format(state[0].covid_infections)}
          </li>
          <li
            className="ranking-list"
            key={`rankinginfections${currentState.ranking_covid_infections}`}
          >
            Ranking: {currentState.ranking_covid_infections}
            /51
          </li>
          <li key={`covid_rate${state[0].fips}`}>
            Rate of COVID infections: {state[0].covid_rate} %
          </li>
          <li
            className="ranking-list"
            key={`rankingcrate${currentState.ranking_covid_rate}`}
          >
            Ranking: {currentState.ranking_covid_rate}
            /51
          </li>
          <li key={`covid_deaths${state[0].fips}`}>
            Number of COVID - related deaths:
            {new Intl.NumberFormat().format(state[0].covid_deaths)}
          </li>
          <li
            className="ranking-list"
            key={`rankingdeaths${currentState.ranking_covid_deaths}`}
          >
            Ranking: {currentState.ranking_covid_deaths}
            /51
          </li>
          <li key={`covid_fatality_rate${state[0].fips}`}>
            Fatality rate of COVID: {state[0].covid_fatality_rate} %
          </li>
          <li
            className="ranking-list"
            key={`rankingcfrate${currentState.ranking_covid_fatality_rate}`}
          >
            Ranking: {currentState.ranking_covid_fatality_rate}
            /51
          </li>
          <h3> Poverty: </h3>
          <li key={`poverty${state[0].fips}`}>
            Rate of Individuals Populatin In Poverty: {state[0].poverty_rate} %
          </li>
          <li
            className="ranking-list"
          ...
          <li key={`foodInsecurity${state[0].fips}`}>
            Household Food Insecurity Rate: {state[0].food_insecurity_rate} %
          </li>
          <li
            className="ranking-list"
    ...
    return (
      <>
        ...
        <div className="search-results"> {statesResults} </div>
      </>
    );
  }
}
