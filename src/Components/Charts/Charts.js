...
export default class Charts extends React.Component {
  ...
  render() {
    const labels = this.context.stateResults.map(
    ...
    const hispanic = this.context.stateResults.map(
    ...
    const foodInsecurity = this.context.stateResults.map(
      (state) => state[0].food_insecurity_rate
    );
    const poverty = this.context.stateResults.map(
      (state) => state[0].poverty_rate
    );
    const covidInfections = this.context.stateResults.map(
      (state) => state[0].covid_infections
    );
    const population = this.context.stateResults.map((state) => state[0].pop);
    const covidDeaths = this.context.stateResults.map(
